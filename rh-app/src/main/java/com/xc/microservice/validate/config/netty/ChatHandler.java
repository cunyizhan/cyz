...
import com.xc.microservice.validate.redis.ChannelKey;
import com.xc.microservice.validate.redis.MySession;
	...
	protected void channelRead0(ChannelHandlerContext ctx, TextWebSocketFrame msg) 
		...
		if (action == MsgActionEnum.CONNECT.type) {
			...
			if (receiverChannel == null) {
				PushMsgService pushMsgService = (PushMsgService)SpringUtil.getBean("pushMsgService");
				pushMsgService.savePushMsgFromFriendToList(receiverId, senderId, chatMsg);
			} else {
			...
			if (msgIdList != null && !msgIdList.isEmpty() && msgIdList.size() > 0) {
				// 批量签收
				userService.updateMsgStatus(msgIdList);
			}
